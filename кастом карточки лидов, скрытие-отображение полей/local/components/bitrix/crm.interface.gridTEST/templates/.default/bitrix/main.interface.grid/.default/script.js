function BxInterfaceGrid(table_id) {
    ...
    this.InitTable = function () {
        ...
        for (i = 0; i < n; i++) {
            ...
            if (i >= 2) {
                ...
                row.onmouseover = function () {
                    _this.HighlightRow(this, true)
                };
                row.onmouseout = function () {
                    _this.HighlightRow(this, false)
                };
        ...
        if (this.isAjaxEnabled) {
            this.bindFormSubmits(document.forms["form_" + this.table_id], BX.delegate(this.OnFormSubmit, this));
            this.bindFormSubmits(document.forms["filter_" + this.table_id], BX.delegate(this.OnFilterSubmit, this));
        }
    };
    ...
    this.OnFormSubmit = function () {
        ...
        if (this.isAjaxEnabled && this.ajaxLoader) {
            this.ajaxLoader.setupForm(document.forms["form_" + this.table_id], this.vars["current_url"]);
        }
    };
    ...
    this.OnFilterSubmit = function () {
        ...
        if (this.isAjaxEnabled && this.ajaxLoader) {
            this.ajaxLoader.setupForm(document.forms["filter_" + this.table_id], this.vars["current_url"]);
        }
    };
                        ...
                        if (this.oColsMeta[col_id].editable == true && this.oEditData[row_id][col_id] !== false) {
                            ...
                            switch (this.oColsMeta[col_id].type) {
                                ...
                                case 'date':
                                    ...
                                    span.appendChild(BX.create('INPUT', {
                                        'props': {
                                            'type': 'text',
                                            'name': name,
                                            'value': data,
    ...
    this.Sort = function (url, by, sort_state, def_order, args) {
        ...
        BX.ajax.get('/bitrix/components' + _this.vars.component_path + '/settings.php?GRID_ID=' + _this.table_id + '&action=savesort&by=' + by + '&order=' + order + '&sessid=' + _this.vars.sessid, function () {
            _this.Reload(url)
        });
    };
    ...
    this.AddFilter = function (fields) {
        ...
        this.ShowFilterSettings(filter,
            function () {
                ...
                var mnuItem = {
                    'ID': 'mnu_' + _this.table_id + '_' + filter_id,
                    'TEXT': BX.util.htmlspecialchars(data.name),
                    'TITLE': _this.vars.mess.ApplyTitle,
                    'ONCLICK': 'bxGrid_' + _this.table_id + '.ApplyFilter(\'' + filter_id + '\')'
                };
