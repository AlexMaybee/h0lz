...
<?endif?>
		<td class="bx-crm-actions-col" width="1%"><a href="javascript:void(0);"
...
foreach($arParams["ROWS"] as $index=>$aRow):
	...
	//TODO табличка выбора товаров
?>	
	

	<tr class="bx-crm-table-body 
			   search_product_table_<?=$aRow['data']['PRODUCT']['ID']?>" 
			   oncontextmenu="return bxGrid_<?=$arParams["GRID_ID"]?>.oActions[<?=$index?>]"<?if($sDefAction <> ''):?> ondblclick="<?=htmlspecialcharsbx($sDefAction)?>"<?endif?>>
<?if($arResult["ALLOW_EDIT"]):?>
	<?
...
<?endif?>
	<?if(is_array($aRow["actions"]) && count($aRow["actions"]) > 0):?>
		<td class="bx-crm-actions-col"><a href="javascript:void(0);"
...
<td<?=($columnClass !== '' ? ' class="'.$columnClass.'"' : '')?><?
	...
	<?
		...
		if($header['id'] == 'QUANTITY'){
			echo $aRow['data']['PRODUCT']['QUANTITY'];
		}
