if (typeof BX.CrmQuoteClientManager === "undefined")
{
	BX.CrmQuoteClientManager = function(settings)
	{
		this.settings = settings;
		if (this.settings["formId"])
			this.settings["formId"] = "form_" + this.settings["formId"];
		this.toggleClientCompanyFields();
	};

	BX.CrmQuoteClientManager.prototype = {
		"syncPersonType": function(type, value)
		{
			if (type === "CONTACT")
				this.settings["contactId"] = parseInt(value);
			else if (type === "COMPANY")
				this.settings["companyId"] = parseInt(value);

			if (this.settings["personType"] === "COMPANY")
			{
				if (type === "COMPANY" && value == 0)
					this.settings["personType"] = "CONTACT";
			}
			else
			{
				if (type === "COMPANY" && value > 0)
					this.settings["personType"] = "COMPANY";
				else
					this.settings["personType"] = "CONTACT";
			}
		},
		"toggleClientCompanyFields": function()
		{
			var fields = this.settings["companySpecifiedClientFields"],
			...
			if (typeof(fields) === "object" && fields.length > 0)
			{
				for (var i in fields)
				{
					...
					{
						fName = "hidden_" + fields[i];
						fElem = BX(fName);
						if (!fElem)
						{
							fName = fields[i].toLowerCase() + "_wrap";
							fElem = BX(fName);
						}
						if (fElem)
						{
							if (personType === "COMPANY")
								fElem.removeAttribute("style");
							else
		...
		"rewriteClientFields": function()
		{
			...
			BX.ajax({
				...
				"data":
				{
					...
					"OPTIONS" : {
						"CLIENT_FIELDS": {
							"CONTACT_ID": this.settings["contactId"],
							"COMPANY_ID": this.settings["companyId"]
						}
					}
				},
				onsuccess: function(data)
				{
					if (typeof(data) === "object" && typeof(data["CLIENT_FIELDS"]) === "object")
					{
						var i, j, k, l, el, clFields = null, found;
						var fields = data["CLIENT_FIELDS"];
						for (i in fields)
						{
							k = i;
							k = k.toString();
							if (k.substr(0, 7) === "CLIENT_")
							{
								if (clFields === null)
								{
									found = BX.findChildren(BX(self.settings["formId"]), {"tag": "input", "attr": {"type": "text"}}, true);
									clFields = {};
									if (found && ((l = found.length) > 0))
									{
										for (j=0; j<l; j++)
										{
											if (found[j].name.toString().substr(0, 7) === "CLIENT_")
												clFields[found[j].name] = found[j];
										}
									}
								}
								el = clFields[k];
...
if(typeof BX.CrmQuoteEditor === "undefined")
{
	...
	BX.CrmQuoteEditor.prototype =
	{
		initialize: function(id, settings)
		{
			...
			this._clientManager = new BX.CrmQuoteClientManager({
				"formId": this._formId,
				"url": settings["url"],
				"personType": settings["personType"],
				"personTypeIds": settings["personTypeIds"],
				"contactId": parseInt(settings["contactId"]),
				"companyId": parseInt(settings["companyId"]),
				"companySpecifiedClientFields": settings["companySpecifiedClientFields"],
		...
		{
			if (typeof(data) === "object" && data.hasOwnProperty("product") && this._opportunityElement)
			{
				var product = data["product"];
				if (typeof(product) === "object" && product.hasOwnProperty("_editor"))
				{
					var prodEditor = product._editor;
					if (typeof(prodEditor) === "object")
		...
		{
			if (type !== "COMPANY" && type !== "CONTACT" || !this._clientManager)
				return;

			this._clientManager.syncPersonType(type, value);
			this._clientManager.toggleClientCompanyFields();
			this._clientManager.rewriteClientFields();
		},
...
$(document).ready(function() {
 $("input[name='CLIENT_PHONE']").mask("38(099)9999999"); 
});

//сумма предоплаты прячем

$(document).ready(function(){
		$("#uf_crm_1472655795_wrap").hide(); // сумма предоалпты на клик

			$("select[name='UF_CRM_1472655783'").change(function(){
	   	if ($(this).val()=='1000') { 
	   		 $("#uf_crm_1472655795_wrap").show();
	   	} else {
	    		$("#uf_crm_1472655795_wrap").hide();
	   	}

});
//без клика
if ($("select[name='UF_CRM_1472655783'").val()=='1000') {
	   		 $("#uf_crm_1472655795_wrap").show();
	   	} else {
	    		$("#uf_crm_1472655795_wrap").hide();
		}
...
$(function() {
	...
	$("input[name=UF_CRM_1472655820], input[name=UF_CRM_1472655807]").on("input", function(){
		
		let 	n1 = parseFloat($("input[name=UF_CRM_1472655820]").val()), 
     		    n2 = parseFloat($("input[name=UF_CRM_1472655807]").val()); 
     	$("input[name=UF_CRM_1472656631]").val(n1*n2);        
