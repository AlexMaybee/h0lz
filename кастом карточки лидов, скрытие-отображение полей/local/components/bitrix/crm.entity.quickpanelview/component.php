...
{
	ShowError('Not find module multidealtype');
	return;
}

$type_dao = new Itlogic\Multidealtype\NewDealTypeDao();
...
if($arResult['ENTITY_TYPE_NAME'] == 'DEAL'){
	$url = explode('/', $arResult['SERVICE_URL']);
	$url[1] = 'local';
	$arResult['SERVICE_URL'] = implode('/', $url);
}
...
elseif($entityTypeID === CCrmOwnerType::Deal)
{
	...
	//TODO in hear need create toolbars

	$deal  = CCrmDeal::GetByID($arResult['ENTITY_ID']);
	$res = CCrmStatus::GetList(['SORT' => 'ASC']);

	$arrs = $type_dao->getStageOnlySelectedType($deal,$res);
