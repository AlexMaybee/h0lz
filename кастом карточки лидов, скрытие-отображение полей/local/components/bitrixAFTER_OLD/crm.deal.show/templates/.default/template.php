...
#############################################################################################################
echo "<a href='".$newUrl."/local/lib/dealAdd.php?OLDDEALID=".$arResult['ELEMENT']['ID']."'>Сгенерировать сделку с поставщиком</a><br>";

// if ($USER->IsAdmin()) {
	$path = "local/components/bitrix/crm.deal.show/templates/.default";
	$dealID = $arResult['ELEMENT_ID'];
	// print_r($arResult);
	echo "
	<form action=\"/$path/genCSV.php?DEAL_ID=$dealID\" method=\"POST\" style=\"margin:5px 0;\">
		<input type=\"submit\" value=\"Сохранить данные о сделке\" name=\"$dealID\">
	</form>
	";

	
	// echo "<a href=\"/$path/genCSV.php\">Сохранить данные о сделке</a>";
// }
#############################################################################################################
