...
// print_r(split('[/.-]', $_SERVER['REDIRECT_SCRIPT_URI']));
// echo "</pre>";
// die();
#######################################################################################################################
function get_contact_user_field_DEAL($id, $field){

    $arFilter = array("ID" => $id);
    $arParams = array($field);
    $by =Array('DATE_CREATE' => 'DESC');
    $arRes = CCrmContact::GetList($by,$arFilter,$arParams);
    $res = $arRes->Fetch();
    return isset($res[$field]) ? $res[$field] : FALSE;
}
$url = split('[/.-]', $_SERVER['REDIRECT_SCRIPT_URI']);
if($url[8] == 'edit' && $url[7] == 'deal'){
		CModule::IncludeModule("crm");
		$CCrmDeal = new CCrmDeal();
		$CCrmContact = new CCrmContact();
		$deal = $CCrmDeal->GetByID($url[9]);
		$contact = $CCrmContact->GetByID($deal['CONTACT_ID']);
		$adr1 = $contact['ADDRESS'].' '.$contact['ADDRESS_2'].' '.$contact['ADDRESS_CITY'].' '.$contact['ADDRESS_POSTAL_CODE'].' '.$contact['ADDRESS_REGION'].' '.$contact['ADDRESS_PROVINCE'].' '.$contact['ADDRESS_COUNTRY'].' '.$contact['ADDRESS_COUNTRY_CODE'];
		$adr2 = get_contact_user_field_DEAL($deal['CONTACT_ID'], 'UF_CRM_1458808398').' '.get_contact_user_field_DEAL($deal['CONTACT_ID'], 'UF_CRM_1458808412').' '.get_contact_user_field_DEAL($deal['CONTACT_ID'], 'UF_CRM_1458808422').' '.get_contact_user_field_DEAL($deal['CONTACT_ID'], 'UF_CRM_1458808430').' '.get_contact_user_field_DEAL($deal['CONTACT_ID'], 'UF_CRM_1458808438').' '.get_contact_user_field_DEAL($deal['CONTACT_ID'], 'UF_CRM_1458808448').' '.get_contact_user_field_DEAL($deal['CONTACT_ID'], 'UF_CRM_1458808457');
		$adr3 = get_contact_user_field_DEAL($deal['CONTACT_ID'], 'UF_CRM_1458808539').' '.get_contact_user_field_DEAL($deal['CONTACT_ID'], 'UF_CRM_1458808547').' '.get_contact_user_field_DEAL($deal['CONTACT_ID'], 'UF_CRM_1458808555').' '.get_contact_user_field_DEAL($deal['CONTACT_ID'], 'UF_CRM_1458808563').' '.get_contact_user_field_DEAL($deal['CONTACT_ID'], 'UF_CRM_1458808571').' '.get_contact_user_field_DEAL($deal['CONTACT_ID'], 'UF_CRM_1458808579').' '.get_contact_user_field_DEAL($deal['CONTACT_ID'], '').' '.get_contact_user_field_DEAL($deal['CONTACT_ID'], '').' '.get_contact_user_field_DEAL($deal['CONTACT_ID'], 'UF_CRM_1458808588');
}
#######################################################################################################################
...
foreach($mainTab['fields'] as &$field):
	...
	if($url[8] == 'edit' && $url[7] == 'deal'){
		...
		// echo "<pre>";
		// print_r($adr1);
		// var_dump(strlen($adr3));
		// echo "</pre>";
		if($field['id'] == 'UF_CRM_1458811248'){
			$field['value'] = '<input type="hidden" name="UF_CRM_1458811248" value="">'
				.'<select class="bx-user-field-enum" name="UF_CRM_1458811248">'
					.'<option selected value="">Нет</option>'
					.'<option value="'.$adr1.'">'.$adr1.'</option>';
					if(strlen($adr2) > 9 ){
						$field['value'] .= '<option value="'.$adr2.'">'.$adr2.'</option>';
					}
					if(strlen($adr3) > 9 ){
						$field['value'] .= '<option value="'.$adr3.'">'.$adr3.'</option>';
					}
				$field['value'] .='</select>';
		}
	}
...
foreach($arSections as &$arSection):
	...
	foreach($arSection['FIELDS'] as &$field):
		...
		elseif($field['type'] === 'multiple_address'):
			...
			$addressCreationTextTemplate = isset($params['CREATION_TEXT_TEMPLATE'])
				? $params['CREATION_TEXT_TEMPLATE'] : GetMessage('CRM_ADDRESS_CREATE');
						...
						function()
						{
							...
							BX.CrmMultipleAddressEditor.create(
								"<?=$fieldNodePrefix?>",
								{
									...
									creationTextTemplate: "<?=CUtil::JSEscape($addressCreationTextTemplate)?>",
