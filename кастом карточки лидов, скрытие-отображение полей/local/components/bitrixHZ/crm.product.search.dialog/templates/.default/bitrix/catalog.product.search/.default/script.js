...
if (typeof(BX.Crm.ProductSearchDialog) === "undefined")
{
	BX.Crm.ProductSearchDialog = (function () {
		...
		var ProductSearchDialog = function (parameters) {
			...
			if (typeof(BX.Crm.ProductSearchDialog.splitterConfig) === "object")
			{
			...
			if (this.leftContainer && typeof(BX.Crm.Splitter) !== "undefined")
			{
				...
				if (container)
				{
					...
					if (typeof(this.splitterConfig) === "object")
					{
		...
		ProductSearchDialog.prototype = {
			...
			SelEl: function (params, scope)
			{
				...
				var item = document.getElementsByClassName('search_product_table_'+params.id);
				var node = document.createElement('TD');
				node.innerHTML = '<div aling="center"> Добавлено. </div>' 
				item[0].innerHTML = '';
		...
		var btn = document.getElementsByClassName('popup-window-close-icon')[0];
				btn.click();
