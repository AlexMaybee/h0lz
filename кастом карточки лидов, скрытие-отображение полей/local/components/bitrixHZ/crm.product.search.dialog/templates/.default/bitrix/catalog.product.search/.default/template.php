...
if(!CModule::IncludeModule('itlogic.warehouse')){
	ShowError("Error not find module warehouse");
	return;
}
...
$first_arr = explode('://',$_SERVER['HTTP_REFERER']);
$last_arr = explode('/',$first_arr[1]);
...
foreach($last_arr as $k=>$v){
	...
	if($v == 'deal'){
		$i = $k+2;
		$type['deal'] = $last_arr[$i];
	}else if($v == 'lead'){
		$i = $k+2;
		$type['lead'] = $last_arr[$i];
	}
}
...
foreach ($arResult['PRODUCTS'] as $productId => $arItems)
{
	...
	if(count($type) > 0){
		...
		$tp = array_keys($type);
		$arItems['PRODUCT']['QUANTITY'] =
			$dao->getAvailableForProductList($arItems['PRODUCT']['ID'],$type[$tp[0]],$tp[0]);
	...
	$gridDataRecord = array(
		...
		'editable' => false,
		'columns' => array(
			'NAME' => '<a class="crm-gds-item'.($arItems['TYPE'] === 'S' ? ' crm-gds-item-section' : ' crm-gds-item-gds').'">' . $arItems['NAME'].'</a>',
