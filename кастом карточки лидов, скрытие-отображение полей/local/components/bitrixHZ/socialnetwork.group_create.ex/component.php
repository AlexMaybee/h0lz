	...
	if (StrLen($arResult["FatalError"]) <= 0)
	{
		...
		)
		{
			...
			if (!array_key_exists("TAB", $arResult) || $arResult["TAB"] == "edit")
			{
				...
				if(IntVal($_POST['SELECTED_PARENT_GROUP']) <=0 ){
					$errorMessage .= "Не указана подгруппа."."<br />";
					$arResult["ErrorFields"][] = "GROUP_SUBJECT_ID";
				}
			...
			)
			{
				...
				if ($arParams["GROUP_ID"] <= 0)
				{
					...
					$selected = $_POST['SELECTED_PARENT_GROUP'];

					$parent = new ParentGroup();

					if($selected != '-'){

						$res = $parent->add($selected, $arResult["GROUP_ID"]);
