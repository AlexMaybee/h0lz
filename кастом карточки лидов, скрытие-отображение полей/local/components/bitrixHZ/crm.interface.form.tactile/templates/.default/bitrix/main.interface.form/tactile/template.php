...
foreach($arSections as &$arSection):
	...
	if($arParams['FORM_ID'] == 'CRM_DEAL_EDIT_V12'){

	}
		
	
	if($arParams['FORM_ID'] == 'CRM_PRODUCT_EDIT'){

		CModule::IncludeModule('itlogic.warehouse');
		$dao = new Itlogic\Warehouse\WarehouseDao();
		
	
		if($arParams['MODE'] == 'EDIT'){

			$v = (isset($arParams['DATA']['ID'])) ? 
			$dao->getAllCountsItems($arParams['DATA']['ID']):0;
			$chebx = 'checkbox';
			$t = 'text';
			$itm = $dao->getItemById($arParams['DATA']['ID']);
			
			$val = ($itm['check']== 1 )?'Y':'N';
			

		}else if ($arParams['MODE'] == 'VIEW'){

			$id = [];
			foreach ($arParams['TABS'] as $key => $value) {
			...
			$itm = $dao->getItemById($id['id']);
			$chebx = 'label';
			$val = ($itm['check']== 1 )?'Да':'Нет';
			$v = (isset($id['id'])) ? $dao->getAllCountsItems($id['id']):0;
			$t = 'label';
		...
		$arSection['FIELDS'][] = $account_checkbox;
		$arSection['FIELDS'][] = $warehouse;
...
$arFieldSets = isset($arParams['FIELD_SETS']) ? $arParams['FIELD_SETS'] : array();
	...
	// echo "<pre>"; 
	// var_dump($arParams['FIELD_SETS']); exit;
	// echo "</pre>"; 
	...
	if($arParams['~BUTTONS']['standard_buttons'] !== false):
		?><div class="webform-buttons ">
			<span class="webform-button webform-button-create">
				...
				<input class="webform-button-text checked-for-item-count" type="submit" name="saveAndView" id="<?=$arParams["FORM_ID"]?>_saveAndView" value="<?=htmlspecialcharsbx(GetMessage('interface_form_save_and_view'))?>" title="<?= htmlspecialcharsbx(GetMessage('interface_form_save_and_view_title'))?>" />
				<span class="webform-button-right"></span>
			</span><?
		if(isset($arParams['IS_NEW']) && $arParams['IS_NEW'] === true):
			?><span class="webform-button">
				<span class="webform-button-left"></span>
				<input class="webform-button-text checked-for-item-count" type="submit" name="saveAndAdd" id="<?=$arParams["FORM_ID"]?>_saveAndAdd" value="<?=htmlspecialcharsbx(GetMessage('interface_form_save_and_add'))?>" title="<?= htmlspecialcharsbx(GetMessage('interface_form_save_and_add_title'))?>" />
				<span class="webform-button-right"></span>
			</span><?
		else:
			?><span class="webform-button">
				<span class="webform-button-left"></span>
				<input class="webform-button-text checked-for-item-count" type="submit" name="apply" id="<?=$arParams["FORM_ID"]?>_apply" value="<?=htmlspecialcharsbx(GetMessage('interface_form_apply'))?>" title="<?= htmlspecialcharsbx(GetMessage('interface_form_apply_title'))?>" />
				<span class="webform-button-right"></span>
			</span><?
		endif;
