...
if(!CModule::IncludeModule('itlogic.warehouse')){
	...
	ShowError("Error. itlogic.warehouse not found!");
	return;
}
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST' && check_bitrix_sessid())
	{
		...
		if(isset($_POST['save']) || isset($_POST['saveAndView']) || isset($_POST['saveAndAdd']) || isset($_POST['apply']))
		{
			...
			if(count($arResult['PRODUCT_ROWS']) > 0 ){
				...
				foreach($arResult['PRODUCT_ROWS'] as $r){

					$dao->reserveItems($r['PRODUCT_ID'],(int)$r['QUANTITY'],$arResult['ELEMENT']['ID'],'deal');
				}
			} 
	...
	elseif (isset($_GET['delete']) && check_bitrix_sessid())
	{
		...
		if ($bEdit)
		{
			...
			if (empty($arResult['ERROR_MESSAGE'])){
				...
				$dao->deleteReserveByType($arResult['ELEMENT']['ID'],'deal');
