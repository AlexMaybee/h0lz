<?
use Itlogic\Warehouse\NewDealTypeDao;
...
if($ownerType != ''){
	...
	switch ($ownerType) {
		case 'D':
			$type_for_warehouse = 'deal';
			break;
		case 'L':
			$type_for_warehouse = 'lead';
			break;	
...
// 'OWNER_TYPE':'D', 'OWNER_ID':7 'PRODUCT_ID':100, 'QTY':1, 'CURRENCY_ID':1 'PRICE':100.50
	...
	if($fields['PRODUCT_ID'] === 0)
	{
		...
		{
			echo CUtil::PhpToJSObject(array('ERROR' => 'CUSTOM_PRODUCT_NAME_NOT_ASSIGNED'));
			die();
		}
	}
...
elseif($mode === 'UPDATE_PRODUCT')
{
	...
	//print_r($_POST); exit;
	$available = $dao->getAvailableItemCount($productID,$ownerID,$type_for_warehouse);
	
	if($available != 'not_check'){

		if((int)$fields['QUANTITY'] > $available){
			
			echo CUtil::PhpToJSObject(array('ERROR'=>'WAREHOUSE_DO_NOT_HAVE_NEEDED_QUANTITY',
											'AVAILABLE'=> $available));
			die();
...
//			if($ownerType == 'D'){
//				$deal = CCrmDeal::GetByID($ownerID);
//				$stage = CCrmStatus::GetList(['ID'=>'ASC'],['ENTITY_ID'=>'DEAL_TYPE']);
//				$check_id = 0;
//				while($r = $stage->Fetch()){
//					if($r['STATUS_ID'] == $deal['TYPE_ID']){
//						$check_id = $r['ID'];
//					}
//				}
...
//			}
			...
			$dao->reserveItems($productID,(int)$fields['QUANTITY'],$ownerID, $type_for_warehouse);
		}
...
elseif($mode === 'SAVE_PRODUCTS')
{
	...
	foreach ($arProducts as $arProduct)
	{
		...
		$fields['QUANTITY'] = isset($arProduct['QUANTITY']) ? doubleval($arProduct['QUANTITY']) : 1.0;
		if($fields['QUANTITY'] <= 0) // Zero and negative quantity are not allowed
		{
			$fields['QUANTITY'] = 1;
		}
...
//		}
		...
		if($available != 'not_check'){
			...
			if((int)$fields['QUANTITY'] > $available){
				...
				die();
					...
					$dao->reserveItems($productID,(int)$fields['QUANTITY'],
						  $ownerID,$type_for_warehouse);
		...
		// Zero and negative prices are allowed
		$fields['PRICE'] = isset($arProduct['PRICE']) ? round(doubleval($arProduct['PRICE']), 2) : 1.0;
...
elseif($mode === 'CALCULATE_TOTALS')
{
	...
	foreach($productRows as &$productRow)
	{
		...
		$productRow['AVAILABLE_COUNT'] = $dao->getAvailableItemCount($productRow['PRODUCT_ID'],$ownerID, $type_for_warehouse);
		...
		if($ownerType == 'D'){
			if($check_id != 0){
				$productRow['AVAILABLE_COUNT'] = 'not_check';
			}
		}

		if($productRow['AVAILABLE_COUNT'] != 'not_check' ){
			if($productRow['QUANTITY'] > $productRow['AVAILABLE_COUNT']){
				echo CUtil::PhpToJSObject(array('ERROR'=>'WAREHOUSE_DO_NOT_HAVE_NEEDED_QUANTITY',
												'AVAILABLE'=> $productRow['AVAILABLE_COUNT']));
				die();
			}	
		}
...
}else if ($mode === 'UNRESERVE_ITEM'){

	$productId = $_POST['PRODUCT_ID'];
	$owner_id = $_POST['OWNER_ID'];
	
	$dao->unReserveItem($productId,$owner_id,$type_for_warehouse);
	echo "OK";
}
die();
?>