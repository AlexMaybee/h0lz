...
if (typeof(BX.CrmProductEditor) === "undefined")
{
	...
	BX.CrmProductEditor = function ()
	{
		...
		this._ajaxErrorMessage = "";

	};
	BX.CrmProductEditor.prototype =
	{

		openModalWindow: function (template){
			

			var id = new Date().getTime();

		    var oPopup = new BX.PopupWindow("id_"+id, window.body, {
		...
		_prepareProductSettings: function(item, product)
		{
			...
			if (discountData)
			{
				...
				var discountRate = typeof(discountData['discountRate']) !== 'undefined' ? Number(parseFloat(discountData["discountRate"]).toFixed(2)) : 0.0;
				var discountSum = typeof(discountData['discountSum']) !== 'undefined' ? Number(parseFloat(discountData["discountSum"]).toFixed(2)) : 0.0;
			}
			...
			if(this.isTaxAllowed())
			{
				...
				settings['TAX_RATE'] = taxInfo ? Number(parseFloat(taxInfo['VALUE']).toFixed(2)) : 0.0;
				var isTaxInPrice = typeof(taxData['included']) !== 'undefined' ? !!taxData['included'] : false;
				if (!this.isInvoiceMode() && nProducts > 0 && this.getSetting("taxUniform", true))
				{
					...
					if (lastProduct !== product || nProducts > 1)
					{
						...
						if (lastTaxIncluded !== isTaxInPrice)
						{
							if (isTaxInPrice)
							{
								settings['PRICE'] = Number(BX.CrmProduct.calculateExclusivePrice(settings['PRICE'], settings['TAX_RATE']).toFixed(2));
							}
				...
				if(!isTaxInPrice)
				{
					settings['PRICE'] = Number(BX.CrmProduct.calculateInclusivePrice(settings['PRICE'], settings['TAX_RATE']).toFixed(2));
				}
				settings['TAX_INCLUDED'] = (this.isInvoiceMode()) ? true : isTaxInPrice;
			}
			else
			{
				var defaultTaxInfo = this.getSetting('defaultTax', null);
				settings['TAX_RATE'] = 0.0/*defaultTaxInfo ? Number(parseFloat(defaultTaxInfo['VALUE']).toFixed(2)) : 0.0*/;
		...
		_addItem: function(item, removeEmptyRow)
		{
			...
			BX.findChildren(row, function(el) {
				if (typeof(el) === "object" && BX.type.isElementNode(el))
				{
					if (el.id)
					{
						var elId = el.id;
		...
		_deleteProduct: function(product)
		{
			...
			var ajax_urls = this._serviceUrl.split('/');
			ajax_urls[1]= 'local';
			this._serviceUrl = ajax_urls.join('/');
			console.log(product);
			console.log("ID = "+product._settings.ID);
			console.log("Quantity = "+product._settings.QUANTITY);
			console.log(this._settings.ownerID);
			console.log(this._settings.ownerType);
				...
				BX.ajax(
				{
					...
					{
						'MODE': 'UNRESERVE_ITEM',
						'PRODUCT_ID':product._settings.PRODUCT_ID,
						'QUANTITY':product._settings.QUANTITY,
						'OWNER_ID':this._settings.ownerID,
						'OWNER_TYPE':this._settings.ownerType
					},
					onsuccess: function(){
		...
		_onCalculateTotalsRequestSuccess: function(data)
		{
			...
			var buttons = document.querySelectorAll('.checked-for-item-count');
				
			for(var i = 0; i < buttons.length; i++ ){
				buttons[i].removeEventListener('click',this.errorEventListener);
				console.log("Remove Listener for "+ buttons[i]);
			}
		...
		refreshTotals: function(totals)
		{
			...
			if(el)
			{
				...
				if (parseFloat(ttl) !== 0.0)
					this._discountExists = true;
				else
					this._discountExists = false;
				el.innerHTML = s !== '' ? s : this._currencyFormat.replace(/#/g, ttl);
		...
		_onCalculateTotalsRequestFailure: function(data)
		{
			...
			BX.CrmProductEditor.prototype._processAjaxError(data);
		},
		...
		errorEventListener: function(e){
			...
			console.log('Start event listener method');
			console.log(document.nz_error_messages);
			console.log('end ========================');
			e.preventDefault();
			BX.CrmProductEditor.prototype.openModalWindow(document.template_message_for_modal);
		...
		_processAjaxError: function(data)
		{
				...
				|| error == 'PRODUCT_ROWS_SAVING_ERROR')
			{
			...
			}else if(error == 'WAREHOUSE_DO_NOT_HAVE_NEEDED_QUANTITY'){
				

				var template = 
				
				'<div style="padding:30px">'+
					'<h1>Превышено допустимое кол-во товара. Доступно ' +data["AVAILABLE"]+'</h1>'+
				'</div>';

				document.nz_error_messages = error;
				document.template_message_for_modal = template;
				...
				for(var i = 0; i < buttons.length; i++ ){
					buttons[i].addEventListener('click',this.errorEventListener);
					console.log("Add Listener for "+ buttons[i]);
				}

				this.openModalWindow(template);
				return true;
...
if (typeof(BX.CrmProduct) === "undefined")
{
	...
	BX.CrmProduct.prototype =
	{
		...
		layout: function()
		{
			...
			// TAX SUM
			this.setFieldValue("TAX_SUM", Number(this.getSetting("TAX_SUM", 0.0)).toFixed(2));
			// SUM
			var showTotal = this.getSetting("PRICE", 0.0) * this.getSetting("QUANTITY", 0.0);
			this.setFieldValue("SUM", showTotal.toFixed(2));
		...
		saveSettings: function()
		{
			...
			//this._settings['PRICE'] = this._parseFloat(this._fieldValue['PRICE'], 2, 0.0);    // always in settings
		},
		...
		setSettings: function(settings)
		{
			...
			this._settings['QUANTITY'] = Number(parseFloat(this.getSetting('QUANTITY', 0.0)).toFixed(4));
			...
			this._settings['DISCOUNT_RATE'] = Number(parseFloat(this.getSetting('DISCOUNT_RATE', 0)).toFixed(2));
			this._settings['DISCOUNT_SUM'] = Number(parseFloat(this.getSetting('DISCOUNT_SUM', 0.0)).toFixed(2));
			this._settings['DISCOUNT_SUBTOTAL'] = Number((this._settings['QUANTITY'] * this._settings['DISCOUNT_SUM']).toFixed(2));

			this._settings['TAX_RATE'] = Number(parseFloat(this.getSetting('TAX_RATE', 0.0)).toFixed(2));
			this._settings['TAX_INCLUDED'] = this.isTaxIncluded();

			this._settings['PRICE'] = Number(parseFloat(this.getSetting('PRICE', 0.0)).toFixed(2));
			...
			var exclusivePrice = Number(BX.CrmProduct.calculateExclusivePrice(this._settings['PRICE'], this._settings['TAX_RATE']).toFixed(2));
			...
			else
			{
				...
				var discountSum = discountRate < 100
					? Number(this._calculateDiscountByDiscountPrice(exclusivePrice, discountRate).toFixed(2))
			...
			{
				this._settings['PRICE_BRUTTO'] = Number(BX.CrmProduct.calculateInclusivePrice(this._settings['PRICE_NETTO'], this._settings['TAX_RATE']).toFixed(2));
			}
		...
		{
			var json = "",
				isTaxAllowed = this._editor.isTaxAllowed();
			if(!this._viewMode)
			{
				this.saveSettings();
			}
		...
		{
			var discount, discountSum, discountPrice, discountSubTotal, exclusivePrice;
			var discountType = this.getDiscountTypeId();
			...
			if(fieldId === 'DISCOUNT_TYPE_ID')
			{
				...
				{
					discountPrice = this._settings['PRICE_NETTO'] - this._settings['DISCOUNT_SUM'];
					this._settings['PRICE'] = Number(BX.CrmProduct.calculateInclusivePrice(discountPrice, this._settings['TAX_RATE']).toFixed(2));
					this._settings['DISCOUNT_RATE'] = Number(this._calculateDiscountRate(this._settings['PRICE_NETTO'], (this._settings['PRICE_NETTO'] - this._settings['DISCOUNT_SUM'])).toFixed(2));
					this.setFieldValue('DISCOUNT_RATE', this._settings['DISCOUNT_RATE']);

					this._settings['DISCOUNT_SUM'] = Number(this._calculateDiscount(this._settings['PRICE_NETTO'], this._settings['DISCOUNT_RATE']).toFixed(2));
				...
				{
					discountPrice = Number(this._calculatePrice(this._settings['PRICE_NETTO'], this._settings['DISCOUNT_RATE'], BX.CrmDiscountType.percentage).toFixed(2));
					this._settings['PRICE'] = Number(BX.CrmProduct.calculateInclusivePrice(discountPrice, this._settings['TAX_RATE']).toFixed(2));
					this._settings['DISCOUNT_SUM'] = Number(this._calculateDiscount(this._settings['PRICE_NETTO'], this._settings['DISCOUNT_RATE']).toFixed(2));
					this.setFieldValue('DISCOUNT_SUM', this._settings['DISCOUNT_SUM'].toFixed(2));

					// discount subtotal
					discountSubTotal = Number((this._settings['QUANTITY'] * this._settings['DISCOUNT_SUM']).toFixed(2));
			...
			else if(fieldId === 'PRICE_BRUTTO' || fieldId === 'PRICE_NETTO')
			{
				if(fieldId === 'PRICE_BRUTTO')
				{
					...
					this._settings['PRICE_NETTO'] = Number(BX.CrmProduct.calculateExclusivePrice(this._settings['PRICE_BRUTTO'], this._settings['TAX_RATE']).toFixed(2));
				...
				if(this._settings['DISCOUNT_TYPE_ID'] === BX.CrmDiscountType.percentage)
				{
					...
					{
						discountPrice = this._settings['PRICE_NETTO'];
					...
					{
						discountPrice = Number(this._calculatePrice(this._settings['PRICE_NETTO'], this._settings['DISCOUNT_RATE'], BX.CrmDiscountType.percentage).toFixed(2));
						this._settings['PRICE'] = Number(BX.CrmProduct.calculateInclusivePrice(discountPrice, Number(this._settings['TAX_RATE']).toFixed(2)));
						discountSum = this._settings['PRICE_NETTO'] - discountPrice;
					}
				...
				else if(this._settings['DISCOUNT_TYPE_ID'] === BX.CrmDiscountType.monetary)
				{
					if(this._settings['DISCOUNT_SUM'] == 0.0)
					{
						discountPrice = this._settings['PRICE_BRUTTO'];
						this._settings['PRICE'] = Number(this._settings['PRICE_BRUTTO'].toFixed(2));
						this.setFieldValue('DISCOUNT_RATE', 0.0);
					}
					else
					{
						discountSum = this._settings['DISCOUNT_SUM'];
						discountPrice = this._settings['PRICE_NETTO'] - discountSum;
						this._settings['PRICE'] = Number(BX.CrmProduct.calculateInclusivePrice(discountPrice, this._settings['TAX_RATE']).toFixed(2));
						this.setFieldValue('DISCOUNT_RATE', this._calculateDiscountRate(this._settings['PRICE_NETTO'], discountPrice));
					}
				}
			...
			else if(fieldId === 'TAX_RATE')
			{
				...
				discountPrice = Number(this._calculatePrice(this._settings['PRICE_NETTO'], discount, discountType).toFixed(2));
				this._settings['PRICE'] = Number(BX.CrmProduct.calculateInclusivePrice(discountPrice, this._settings['TAX_RATE']).toFixed(2));
			...
			else if(fieldId === 'DISCOUNT_SUBTOTAL')
			{
				...
				if(Number(parseFloat(this._settings['QUANTITY']).toFixed(4)) === 0.0)
				{
				...
				this._settings['DISCOUNT_SUM'] = Number((this._settings['DISCOUNT_SUBTOTAL'] / this._settings['QUANTITY']).toFixed(2));
				this.setFieldValue('DISCOUNT_SUM', this._settings['DISCOUNT_SUM'].toFixed(2));

				discountPrice = this._settings['PRICE_NETTO'] - this._settings['DISCOUNT_SUM'];
				this._settings['PRICE'] = Number(BX.CrmProduct.calculateInclusivePrice(discountPrice, this._settings['TAX_RATE']).toFixed(2));

				exclusivePrice = Number(BX.CrmProduct.calculateExclusivePrice(this._settings['PRICE'], this._settings['TAX_RATE']).toFixed(2));
				this._settings['DISCOUNT_RATE'] = Number(this._calculateDiscountRate(this._settings['PRICE_NETTO'], exclusivePrice).toFixed(2));
				this.setFieldValue('DISCOUNT_RATE', this._settings['DISCOUNT_RATE']);

				// discount subtotal
				this._fieldValue['DISCOUNT_SUBTOTAL'] = (this._settings['QUANTITY'] * this._settings['DISCOUNT_SUM']).toFixed(2);
				this.setFieldView('DISCOUNT_SUBTOTAL', this._fieldValue['DISCOUNT_SUBTOTAL']);
				this._settings['DISCOUNT_SUBTOTAL'] = Number(this._fieldValue['DISCOUNT_SUBTOTAL']);
			...
			else if(fieldId === 'SUM')
			{
				...
				this._settings['PRICE'] = Number((this._settings['SUM'] / this._settings['QUANTITY']).toFixed(2));
				...
				this._settings['DISCOUNT_SUM'] = Number(discountSum.toFixed(2));
				this.setFieldValue('DISCOUNT_SUM', this._settings['DISCOUNT_SUM']);

				this._settings['DISCOUNT_RATE'] = Number(this._calculateDiscountRate(this._settings['PRICE_NETTO'], exclusivePrice).toFixed(2));
				this.setFieldValue('DISCOUNT_RATE', this._settings['DISCOUNT_RATE']);

				this._settings['PRICE_NETTO'] = Number((exclusivePrice + this._settings['DISCOUNT_SUM']).toFixed(2));
				this._settings['PRICE_BRUTTO'] = Number(BX.CrmProduct.calculateInclusivePrice(this._settings['PRICE_NETTO'], this._settings['TAX_RATE']).toFixed(2));
		...
		_calculateDiscountRate: function(originalPrice, price)
		{
			...
			return Number(((100 * (originalPrice - price)) / originalPrice).toFixed(2));
		},
		...
		_calculateTaxSumByIncusivePrice: function(inclusivePrice, taxRate, quantity)
		{
			inclusivePrice = Number(inclusivePrice).toFixed(2);
			return (inclusivePrice - Number(inclusivePrice / (1 + taxRate / 100)).toFixed(2)) * quantity;
		},
		getTaxSum: function()
		{
			return Number(
				this._calculateTaxSumByIncusivePrice(
					this._settings['PRICE'],
					this._settings['TAX_RATE'],
					this._settings['QUANTITY'])).toFixed(2);
		},
...
//				}
		...
		handleSetProductFromDialog: function(data)
		{
			...
			if(discountTypeId === BX.CrmDiscountType.percentage)
				this.setFieldValue("DISCOUNT_RATE", Number(parseFloat(discountValue).toFixed(2)), true);
			else
		...
		showWindow: function ()
		{
			this.popup = new BX.PopupWindow(
				...
				null,
				{
					...
					"titleBar":
					{
						"content": BX.create("SPAN", { "attrs": { "className": "bx-crm-dialog-title-text" }, "text": BX.CrmProductEditorMessages["productSearchDialogTitle"] })
					},
					events:
					{
